    ...
    def do_POST(self):
        ...
        for field in form.keys():
            ...
            if field_item.filename:
                ...
                # Save file
                with open(str(self.path[1:]), "wb+") as f:
                  f.write(file_data)
                del file_data
                out.write(
                    '\tUploaded {} as {!r} ({} bytes)\n'.format(
...
if __name__ == '__main__':
    ...
    print('Starting server on http://0.0.0.0:8080, use <Ctrl-C> to stop')
    print('Powershell POST: curl.exe -v -X POST -F datafile="@file.txt" http://<ip>:8080/file.txt')
